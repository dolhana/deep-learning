...
class NeuralNetwork(object):
    def __init__(self, input_nodes, hidden_nodes, output_nodes, learning_rate):
        ...
        self.weights_input_to_hidden = \
            np.random.normal(0.0, self.input_nodes**-2.,
    ...
    def train2(self, features, targets):
        ''' Train the network on batch of features and targets.
    ...
    def forward_pass_train(self, X):
        ...
        # signals into final output layer (m, n_h) * (n_h, n_o) = (m, n_o)
    ...
    def backpropagation(self, final_outputs, hidden_outputs, X, y,
        ...
        # actual output.
        error = y - final_outputs  # (m, n_o)

        # TODO: Calculate the hidden layer's contribution to the error
        # (m, n_o) * (n_h, n_o).T = (m, n_h)
        ...
        # print('hidden_error_term.shapa:', hidden_error_term.shape)

        # Weight step (input to hidden)
        # (m, n_i) * (m, n_h)
        delta_weights_i_h += np.sum(
            X[:, :, np.newaxis] * hidden_error_term[:, np.newaxis, :],
            axis=0)
        # Weight step (hidden to output)
        # (m, n_h, 1) * (m, 1, n_o)
        delta_weights_h_o += np.sum(
